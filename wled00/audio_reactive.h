...
float fftPinkNoiseAvg[16] = {0.0};
bool run = true;
int round1 =0 ;
int index1 =0;
void recordPinkNoiseAvg() {
  if(index1 >= 16) return;

  
      if(round1 <= 500) {
        fftPinkNoiseAvg[index1]+=fftResult[index1];
        round1++;
      } else {
        fftPinkNoiseAvg[index1]/=500;
        round1 = 0;
        index1++;
      }
    ...
    for(int j =0; j<16; j++){
        ...
        Serial.print(fftPinkNoiseAvg[j]); Serial.print(" ");
      }
...
void FFTTunedcode( void * parameter) {
    double beatSample = 0;
    double envelope = 0;
    uint16_t rawMicData = 0;
